...
export const startAddProduct =(formdata,resetForm,handelToggle)=>{
    return (dispatch) => {
    axios.post('https://dct-pos-app.herokuapp.com/api/products',formdata,
...
export const startGetProductdata = ()=>{
    return(dispatch)=>{
      axios.get('https://dct-pos-app.herokuapp.com/api/products',
      {headers:{Authorization: "Bearer " + localStorage.getItem("token") }})
      .then(res=>{
          const result = res.data;
          //console.log(result)
...
export const startDeleteProduct = (id) => {
    return (dispatch) => {
        axios.delete(`https://dct-pos-app.herokuapp.com/api/products/${id}`,
        {headers:{Authorization: "Bearer " + localStorage.getItem('token')}})
        .then(res=>{
            const result = res.data;
            //console.log(result)
...
export const startEditProduct = (id,formData,resetForm,handelToggle) => {
  return (dispatch) => {
      axios.put(`https://dct-pos-app.herokuapp.com/api/products/${id}`,formData,
      {headers:{Authorization: "Bearer " + localStorage.getItem('token')}}
      )
      .then(res=>{
        const result = res.data;
